...
# Per-project lock registry — prevents duplicate concurrent Apify/Claude fetches
...
class MetaAdLibraryService:
    ...
    async def get_competitor_ads_cached(
        ...
        A per-project asyncio.Lock prevents duplicate concurrent fetches (and the
        resulting UNIQUE constraint failure on CompetitorResearchCache).
        """
        from app.models.competitor_cache import CompetitorResearchCache

        # Acquire per-project lock before any cache check or fetch
        lock = _fetch_locks.setdefault(project.id, asyncio.Lock())
        async with lock:
            cutoff = datetime.now(timezone.utc) - timedelta(hours=48)

            # Check cache (re-checked inside lock so a second waiter gets the fresh result)
